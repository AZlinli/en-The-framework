...
- (void)addTag:(NSString *)tagStr
{
    ...
    tagButton.isNeedVisinLeft = self.needVisinLeft;
...
{
    [self.lastBtn.layer setBorderColor:_borderColor.CGColor];
    button.layer.borderColor = _selectBorderColor ? _selectBorderColor.CGColor : _borderColor.CGColor;
    self.lastBtn = button;
    if (_clickTagBlock) {
...
- (void)deletAllTags
{
    ...
    {
        for (NSString *tag in self.tagArray.copy)
        {
            [self deleteTag:tag];
        }
    }
...
{
    YZTagButton *selectBnt = self.tagButtons[index];
    [self clickTag:selectBnt];
}

- (void)setNoSelectStatus
{
     self.lastBtn.layer.borderColor =  _borderColor.CGColor;
}
